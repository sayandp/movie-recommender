    ...
    for movie in scores:
        if sim_sums[movie] > 0 and rating_counts[movie] >= 2:
            ...
            if popularity_penalty:
                pred = pred / math.log1p(popularity_map.get(movie, 1))
...
def recommend_movie_ids(user_id: int, n: int = 10, popularity_penalty: bool = False) -> List[int]:
    if user_id not in user_item.index:
        return [mid for mid, _ in _popular_movies[:n]]
    return [mid for mid, _ in _compute_predictions(user_id, n, popularity_penalty=popularity_penalty)]
...
def precision_at_k(user_id: int, k: int = 10, threshold: float = 4.0) -> float:
    ...
    if not liked:
        return 0.0
    return round(len(set(recommend_movie_ids(user_id, n=k)) & liked) / k, 4)
...
def recall_at_k(user_id: int, k: int = 10, threshold: float = 4.0) -> float:
    ...
    if not liked:
        return 0.0
    return round(len(set(recommend_movie_ids(user_id, n=k)) & liked) / len(liked), 4)
